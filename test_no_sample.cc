...
void resampler32NSX(char* szFileIn, char* szFileOut, int nSample, int nMode) {
  ...
  do {
    ...
    if (gcount_size < 640) { memset(in_buffer + gcount_size, 0, 640 - gcount_size); }



    res = ans->process(in_buffer, in_size, ans_buffer, in_size);
        out_stream.write((char*)ans_buffer, gcount_size);

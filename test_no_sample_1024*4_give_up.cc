...
void resampler32NSX(char* szFileIn, char* szFileOut, int nSample, int nMode) {
  ...
  do {
    ...
    //   std::cout << "process index: " << index++ << " insize: " << in_size << " outsize:" << ans_size << " res: " <<
    //   res
    //             << std::endl;
    //         memcpy(output_ptr, samplePatchOut, remained_size);
    //   out_stream.write((char*)output_ptr, remained_size);
    // }
    input_ptr += remained_size;
    remained_all += remained_size;

  } while (in_stream && !in_stream.eof());
  char remained_all_zero[remained_all] = {0};
  out_stream.write(&remained_all_zero[0], remained_all);
...
int main(int argc, char* argv[]) {
  resampler32NSX("/home/fengmao/cowa/webRTCtest/capture.pcm",
                 "/home/fengmao/cowa/webRTCtest/build/capture_NS_no_sample_1024*4_give_up.pcm", 32000, 2);
  printf("�������棬�������...\n");
  return 0;
}
